...
`wgf_init`, `wgf_assets`, `wgf_develop` and `wgf_verification` exist so far, so a whole run still needs `--mock`.
A module owns its domain logic; the engine owns orchestration — a module never edits
