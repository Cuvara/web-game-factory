...
"gameplay|works in title:prototype, title:production|Implements core mechanics, game loop, systems and progression from the development plan, and writes the prototype report. Use when building prototype or production tasks.|core/roles/implementers.md;core/lifecycle/stages/prototype.md;core/lifecycle/stages/production.md;core/artifacts/prototype-report.schema.json;core/artifacts/review-report.schema.json|Work the plan's tasks in dependency order and satisfy both acceptance criteria and tests. When the brief opens with blockers from code review or verification, fix those first. Scope, monetization, platform strategy, core gameplay and architecture change only through the production change process."
