...
"qa|release:qa|Independently verifies a built release candidate and produces the QA report read at gate G5. Use when a release is in QA, or when prototype evidence needs its numbers checked at G4.|core/roles/qa.md;core/lifecycle/stages/qa.md;core/artifacts/qa-report.schema.json;core/templates/qa-report.md|You verify work you did not author, and you may fail a build its author believes is finished. Every defect needs reproduction steps. verdict is derived from blocking defects and performance budgets."
...
"qa|qa|Test suites, device matrices, defect triage, and performance measurement.|core/lifecycle/stages/qa.md;core/artifacts/qa-report.schema.json"
