...
__all__ = ["BASE_NAMES", "BASE_PREFIXES", "scrubbed", "passthrough", "ConfigError"]
...
class ConfigError(ValueError):
    """`factory.agents.env_passthrough` is malformed. Not retryable."""
...
def passthrough(config):
    ...
    listed = ((config or {}).get("agents") or {}).get("env_passthrough") or []
    if not isinstance(listed, list) or not all(isinstance(e, str) and _ENTRY.match(e)
                                               for e in listed):
        raise ConfigError("factory.agents.env_passthrough must be a list of environment "
