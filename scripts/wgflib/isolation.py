...
def guarded_paths(config=None):
    ...
    against the Factory root, never the cwd. Raises ValueError on a malformed list."""
    ...
    return [os.path.normpath(p if os.path.isabs(p) else os.path.join(paths.ROOT, p))
            for p in listed]
